...
def read_EEG_stream(fs=settings.samplingRate, max_buf=settings.maxBufferTime):
    ...
    inlet_EEG = StreamInlet(streamsEEG[0], max_buflen=max_buf,processing_flags=1)
    store_EEG = data_init(fs, 'EEG') # Initialize object
    channel_lst = settings.channelNames
    # I remove this because I do not think we have this in our stream -VTD-
...
def read_marker_stream(stream_name='MyMarkerStream3'):
    ...
    if index_lsl:
        ...
        inlet_marker = StreamInlet(streams[index_lsl],processing_flags=1)
...
def get_epoch(inlet_EEG, inlet_marker, store_EEG, store_marker, user_id, excess_EEG=[], excess_EEG_time=[], excess_marker=[], excess_marker_time=[], state='stable', look_for_trigger=1, tmax=1, fs=500):
    ...
    while look_for_epoch:
        ...
        if len(excess_EEG): 
            ...
            # VTD edit: it seems that the EEG samples after the first image is empty.
            # This may fix it...
            timestamp_EEG = timestamp_marker
        ...
        if len(sample_marker): # if marker is present
            ...
            if avail_samples >= s_epoch: # if one EEG epoch has been recovered
                ...
                time.sleep(np.max([0,((s_epoch-avail_samples)/fs)-0.02]))
                look_for_trigger = 0
